...
/*
- Created by Torin
- This script is to be executed as a cron job every 5 minutes or so
- Implemented with PHP for its direct access to MySQL
- Any text output from this script is completely optional for debugging purpose only

- How it works:
- This script goes through all modules and call their generic fetch/update function
...
*/
	...
	*/
	...
	while ($one_record = mysql_fetch_assoc($sql_result_triggers))
	{
		...
		echo "        <li><strong>$module_name</strong> - $module_type_string - $module_description</li>\n";
	}
	$sql_result_triggers = null;
	echo "    </ul>\n";
	
	//-----------------------------------------------------------
	// Execute the headless triggers
	
	echo "-----------------------------------------------------------<br/>\n";
	echo "	Perform triggers:<br/>\n";
	echo "-----------------------------------------------------------<br/>\n";
	$pretty_end_string = "        </li>\n\n";
	echo "    <ul>\n\n";
	foreach ($modules_array as $module_id => $one_module)
	{			
		//try loading module, skip if fail, print out pretty output along the way
		echo "        <li>\n";
		echo "            <span class='load_module'>\n";
		$module_loaded = load_module( $one_module, true );
		...
		//Print out number of triggers/triggers for this module
		$number_of_triggers = $module_object->getNumberOfTriggers();
		$number_of_headless_triggers = $module_object->getNumberOfHeadlessTriggers();
		if ($number_of_triggers == null || $number_of_triggers <= 0) {
			echo "            module has no triggers/triggered. Skipping... <br/>\n";
			echo "            <span class='warning'>WARNING:</span> Please create at least 1 trigger/trigger for this module.<br/>\n";
			echo "            </span>\n";
			echo $pretty_end_string;
			continue;
		}
		echo "                module has " . $number_of_triggers . " possible triggers(s)<br/>\n";
		echo "                module has " . $number_of_headless_triggers . " headless triggers(s)<br/>\n";
		echo "            </span>\n";
		
		//Perform headless trigger
		echo "            ";
		$event_ids = $module_object->trigger_headless();
		if (count($event_ids) <= 0)
			echo "            module generated no headless trigger<br/>\n";

		//Array of none headless trigger IDs
		$none_headless_trigger_array = $module_object->getNoneHeadlessTriggerIDArray();
		
		//No none-headless triggers to perform
		if (count($none_headless_trigger_array) <= 0) {
			echo "            module generated no param-based trigger<br/>\n";
			echo $pretty_end_string;
			continue;
		}
		
		//Retrieve all user's rules with these trigger IDs
		$trigger_rule_array = array();
		$sql_result_rules = func_getRulesByUserIDAndTriggerIDs($user_id, $none_headless_trigger_array);
		while ($sql_result_rules != null && $one_record = mysql_fetch_assoc($sql_result_rules))
		{
			$rule_id = $one_record['rule_id'];
			$trigger_rule_array[$rule_id] = $one_record;
		}
		$sql_result_rules = null;
				
		//Perform trigger with parameters
		foreach ($trigger_rule_array as $rule_id => $one_rule)
		{
			$rule_id = $one_rule['rule_id'];
			$rule_trigger_id = $one_rule['trigger_id'];
			$the_rule = $one_rule;
			$rule_trigger_param = $the_rule['trigger_param'];
			
			//This is only applicable to param-based triggers: no param, no go
			if ($rule_trigger_param == null || strlen($rule_trigger_param) < 5)
				continue;
			
			echo "            perform trigger_id $rule_trigger_id with params: $rule_trigger_param<br/>\n";
			echo "            ";
			$trigger_id = $module_object->trigger($rule_trigger_id, $rule_trigger_param, $rule_id);
			$event_ids = $trigger_id;
		}
	...
	//-----------------------------------------------------------
	// Phase 2, retrieve all new events in the global events queue & filter them
	//-----------------------------------------------------------
	
	echo "-----------------------------------------------------------<br/>\n";
	echo "Process events<br/>\n";
	echo "-----------------------------------------------------------<br/>\n";
	...
	//-----------------------------------------------------------
	// Retrieve new events in queue for this user
	
	$sql_result_queue = func_getEventsByUserID($user_id);
	...
	if ($num_queue <= 0) {
		echo "<strong>No events to process. Done!</strong><br/>\n";
		die();
	}
	...
	if ($num_rules <= 0) {
		...
		for ($event_index=0; $event_index < count($event_array); $event_index++)
			func_updateEventStatus($event_id, $event_ignored_rule_status_status);
	...
	for ($event_index=0; $event_index < count($event_array); $event_index++)
	{
		...
		foreach ($rule_array as $rule_id => $one_rule)
		{
			...
			if ($filter_passed == false)
			{
				...
				{
					func_updateEventStatus($event_id, $event_ignored_error_filter_module_status);
				...
				if ($number_of_filters == null || $number_of_filters <= 0) {
					func_updateEventStatus($event_id, $event_ignored_error_filter_module_no_filter_status);
				...
				if ($filter_passed == false) {
					func_updateEventStatus($event_id, $event_filtered_status);
					echo "            </span>\n";
					echo $pretty_end_string;
					continue;
					//next rule
			...
			if ($action_ok == false)
			{
				...
				{
					func_updateEventStatus($event_id, $event_ignored_error_action_module_status);
			...
			if ($action_result == null) {
				func_updateEventStatus($event_id, $event_ignored_error_action_error_status);
				continue;
				//next rule
			}
						
			//Action success, flag as done
			func_updateEventStatus($event_id, $event_done_action_status);
			echo $pretty_end_string;
